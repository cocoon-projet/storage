...
class DateComparator
{
    ...
    private const DATE_PATTERN = '#(>|>=|<|<=|==|last|after|before)\s*([0-9]{4}-[0-9]{2}-[0-9]{2}|[0-9]+\s*(minute|hour|day|week|month|year)s?)?#';
    ...
    private function dateComparison(int $file_date, string $date_expression): bool
    {
        ...
        if (isset($matches[2]) && preg_match('#([0-9]+)\s*(minute|hour|day|week|month|year)s?#', $matches[2], $time_matches)) {
